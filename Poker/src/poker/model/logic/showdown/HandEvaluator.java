...
public class HandEvaluator {
    ...
    /**
     * The position of the player. Used for creating the {@link WinningCards} object.
     */
    ...
    public HandEvaluator(int player, Card[] holeCards, Card[] board) {
        ...
        /*
         * The checks for Straight, Flush and Straight Flush don't need the other checks. So perform
         * those checks first!
         */
        ...
        /*
         * The checks for Pairs, Trips, FullHouse and FourOfAKind are next in line. If any of them
         * succeeds, the check for HighCard isn't needed anymore!
         */
        checkForPairings();
        if (!strength.isEmpty()) return;
    ...
    /**
     * Check for {@link HandStrength#OnePair}. If it succeeds, continue with the checks for
     * {@link HandStrength#TwoPair} and {@link HandStrength#Trips}.
     */
    private void checkForPairings() {
        /* This HashMap contains one entry for each rank of the hand and the times it occurs. */
    ...
    private static void removeRankFromList(int rank, ArrayList<Card> hand) {
        for (int i = hand.size() - 1; i >= 0; --i) {
            if (hand.get(i).getRank() == rank) hand.remove(i);
        }
    }
    ...
    private boolean checkForStraight(TreeMap<HandStrength, int[]> handStrength, ArrayList<Card> hand) {
        ...
        if (hand.size() >= 5) {
            ...
            if (hand.size() >= 5 && hand.get(0).getRank() == 12) {
                ...
                for (int i = 0; i < 4; ++i) {
                    if (3 - i != hand.get(i).getRank()) break;
    ...
    /**
     * Check for {@link HandStrength#StraightFlush}. Will only be performed, if the checks for
     * {@link HandStrength#Flush} and {@link HandStrength#Straight} were successful.
     * 
